...
import { LOBBY_COLLAB_BASED_SECTIONS } from '../constants';
...
export async function createDirectoryIfNotExists(name: string, spaceFolder: string) {
    const dirPath = path.join('./src/naNewLobbyDesign/data', name, spaceFolder);
...
export const executeContentfulExport = async (contentType: ContentType, spaceFolder: string, spaceId: string, accessToken: string, env: string) => {
    ...
    // await createFile(`./src/naNewLobbyDesign/data/${name}/${spaceFolder}/${name}-error.log`);
    // await createFile(`./src/naNewLobbyDesign/data/${name}/${spaceFolder}/${name}.json`);

    await contentfulExport({
        spaceId,
        managementToken: accessToken,
        environmentId: env,
        contentFile: `./src/naNewLobbyDesign/data/${name}/${spaceFolder}/${name}.json`,
        skipContentModel: true,
        queryAssets: [],
        queryEntries,
        contentOnly: true,
        errorLogFile: `./src/naNewLobbyDesign/data/${name}/${spaceFolder}/${name}-error.log`,
