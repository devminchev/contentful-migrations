...
export const script = async (spaceLocale: string) => {
    try {
        ...
        for (let index = 0; index < metadata.length; index++) {
            ...
            logSection(`Metadata Excel Progress: (${percentage.toFixed(2)}%)`, game.__EMPTY);

            if (!game.__EMPTY_4 || !game.__EMPTY_2) {
                logSection(`Skipping game due to missing gameType or gameProvider: ${game.__EMPTY}`);
                continue;
            }
            ...
            for (let v2Entry of gameV2Entries) {
                ...
                try {
                    ...
                    logSection(`Error updating and publishing entry: ${v2Entry.sys.id}`, error);
                }
            }
        }
    } catch (error) {
