...
import { DX_PLACEHOLDER, IG_GRID_C_SECTION, SITE_GAME_V2, UNMATCHED_WHITEHAT_GAMES, VENTURE } from "../constants";
...
export const transform = async (spaceLocale: string, spaceFolder: string) => {
    let gridCSectionEntries = [];
    let dxPlaceholderEntryID_newTitle_entires = {};
    let view_key_jur_entries = {};
    ...
    try {
        ...
        const dxGridCPlaceHolder = dxPlaceHolder.entries.filter(entry => {
            const entryTitle = entry.fields.entryTitle[spaceLocale];
            return entryTitle.includes("(casino-grid-c)");
        });

        for (const item of dxGridCPlaceHolder) {
            ...
            const filteredEntryTitle = filterDxEntryTitle(entryTitle, jurisdiction, venture, ventureWithJurisdiction, `(casino-grid-c)`);
            ...
            const payload = {
                fields: {
                    ...
                    game: {[spaceLocale]: foundGamesIds[0] },
                    venture: { [spaceLocale]: { "sys": { "type": "Link", "linkType": "Entry", "id": ventureId } } },
                    image: { [spaceLocale]: image },
                    layoutType: createLayoutType(spaceLocale, 'grid-c'),
            ...
            gridCSectionEntries.push(payload);
        }
        ...
        await storeFile(gridCSectionEntries, `./src/naNewLobbyDesign/data/${DX_PLACEHOLDER}/${spaceFolder}/${IG_GRID_C_SECTION}.json`);
    } catch (error) {
