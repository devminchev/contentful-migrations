...
export const executeContentfulExport = async (contentType: ContentType, spaceFolder: string, spaceId: string, accessToken: string, env: string) => {
    ...
    // await createFile(`./src/euNewLobbyDesign/data/${name}/${spaceFolder}/${name}-error.log`);
    // await createFile(`./src/euNewLobbyDesign/data/${name}/${spaceFolder}/${name}.json`);

    await contentfulExport({
        spaceId,
        managementToken: accessToken,
        environmentId: env,
        contentFile: `./src/euNewLobbyDesign/data/${name}/${spaceFolder}/${name}.json`,
        skipContentModel: true,
        queryAssets: [],
        queryEntries,
        contentOnly: true,
        errorLogFile: `./src/euNewLobbyDesign/data/${name}/${spaceFolder}/${name}-error.log`,
