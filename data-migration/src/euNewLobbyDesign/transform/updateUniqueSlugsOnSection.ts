...
export const updateUniqueSlugsOnSection = async (spaceLocale: string, spaceFolder: string) => {
    ...
    try {
        ...
        for (const igModel of igSectionModels) {
            ...
            await storeFile(updatedEntries, `./src/euNewLobbyDesign/data/${igModel}/${spaceFolder}/${igModel}.json`);
            await storeFile(sortableUniqueSlugsData, `./src/euNewLobbyDesign/data/section/${spaceFolder}/uniqueSlugs.json`);
        }
    } catch (error) {
