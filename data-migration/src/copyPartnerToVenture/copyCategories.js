...
export const getAllVentureCategories = (envvv) => {
    const promises = [];
...
const copyCategories = (async (environment, locale, venture, sourceVenture, partner, targetPartner) => {
  try {
    ...
    // @ts-ignore
    await writeFile(`${ __dirname }/categoryIDMap.json`, JSON.stringify(categoryIDMap, null, 2), err => { console.error(err) });
  } catch (e) {
   console.error(e)
  }
})
