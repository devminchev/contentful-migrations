import { readExcelFile } from '../utils/fileOperations.js';
import { getEntries } from '../api/managementApi.js';
import { logSection, log } from '../utils/logging.js';
import { SYMBOL_TYPE_MAP, FEATURES_MAP, THEMES_MAP } from './constants.js';

const script = async (spaceLocale) => {
    try {
        ...
        for (let index = 0; index < metadata.length; index++) {
            ...
            log(`Metadata Excel Progress: (${percentage.toFixed(2)}%) Processing game: ${game.__EMPTY}`);
            ...
            for (let v2Entry of gameV2Entries) {
                ...
                try {
                    ...
                    log(`Error updating and publishing entry: ${v2Entry.sys.id} Error: ${error.message}`);
                }
            }
        }
    } catch (error) {
