...
export default async (new_model, spaceFolder, spaceLocale, targetEnv, targetSpace, old_model, accessToken = 'YOUR_ACCESS_TOKEN_HERE') => {
    try {
        const client = contentful.createClient({ accessToken });
        const space = await client.getSpace(targetSpace);
        const environment = await space.getEnvironment(targetEnv);

        const data = JSON.parse(await readFile(`./src/euRegulatoryMinMaxBetRemovalGameSiteGame/data/${old_model}/${spaceFolder}/${new_model}.json`, 'utf-8'));
        const dataEntries = [...data.entries];
        console.log('--------------------------------');
        console.log(`PUBLISHING TRANSFORMED ${new_model} ENTRIES`);
        console.log('--------------------------------');
...
async function processEntry(environment, dataEntry, index, total, startTime, spaceLocale) {
    try {
