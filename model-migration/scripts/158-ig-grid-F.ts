...
export = ((migration) => {
    const igGridFSection = migration
        .createContentType('igGridFSection')
        .name('IG Grid F Section')
        .description('')
        .displayField('entryTitle');

    const igGridFFields: { id: string, options: IFieldOptions }[] = [
        ...
        { id: 'layoutType', options: { name: 'Layout Type', type: 'Symbol', required: false, defaultValue: { [locale]: 'grid-f' }, validations: [{ in: ['grid-f'] }], disabled: true } },
    ...
    igGridFFields.forEach(field => {
        const { id, options } = field;
        const fieldInstance = igGridFSection.createField(id).name(options.name as string).type(options.type as FieldType);
    ...
    const fieldControls = [
        { id: 'entryTitle', widget: 'singleLine', helpText: 'The title of the IG Grid F Section entry' },
        { id: 'title', widget: 'singleLine', helpText: 'Localized title for the IG Grid F Section' },
        ...
        { id: 'platformVisibility', widget: 'checkbox', helpText: 'Platforms where this IG Grid F Section is visible' },
        { id: 'sessionVisibility', widget: 'checkbox', helpText: 'Sessions where this IG Grid F Section is applicable' },
        ...
        { id: 'layoutType', widget: 'dropdown', helpText: 'Layout type (fixed to grid-f)' },
    ...
    fieldControls.forEach(({ id, widget, helpText }) => {
        igGridFSection.changeFieldControl(id, 'builtin', widget as BuiltinEditor, { helpText });
    });
