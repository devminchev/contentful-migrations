...
export = (function (migration) {
    ...
    const igCarouselB = migration
        .createContentType('igCarouselB')
        .name('IG Carousel B')
        .description('Our standard carousel, used for recently played.')
        .displayField('entryTitle');

    // 2) Define fields and their options
    const igCarouselBFields: { id: string; options: IFieldOptions }[] = [
        {
            id: 'entryTitle',
            options: {
                name: 'Entry Title',
        ...
        },
        {
            ...
            options: {
                ...
                type: 'Symbol',
                required: false,
                defaultValue: { [locale]: 'carousel-b' },
                validations: [
                    {
                        in: ['carousel-b'],
                    },
    ...
    // 3) Create each field
    igCarouselBFields.forEach((field) => {
        const { id, options } = field;
        const fieldInstance = igCarouselB
            .createField(id)
    ...
    const fieldControls = [
        {
            id: 'entryTitle',
            widget: 'singleLine',
            helpText: 'The title of the IG Carousel B entry',
        },
        {
            id: 'title',
            widget: 'singleLine',
            helpText: 'Localized title for the IG Carousel B',
        },
        ...
        },
        {
            id: 'layoutType',
            widget: 'dropdown',
            helpText: 'Layout type (fixed to carousel-b)',
        },
    ...
    fieldControls.forEach(({ id, widget, helpText }) => {
        igCarouselB.changeFieldControl(id, 'builtin', widget as BuiltinEditor, { helpText });
    });
