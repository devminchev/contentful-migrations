...
export = ((migration) => {
    const igGridDSection = migration
        .createContentType('igGridDSection')
        .name('IG Grid D Section')
        .description('')
        .displayField('entryTitle');

    const igGridDFields: { id: string, options: IFieldOptions }[] = [
        ...
        { id: 'layoutType', options: { name: 'Layout Type', type: 'Symbol', required: false, defaultValue: { [locale]: 'grid-d' }, validations: [{ in: ['grid-d'] }], disabled: true } },
    ...
    igGridDFields.forEach(field => {
        const { id, options } = field;
        const fieldInstance = igGridDSection.createField(id).name(options.name as string).type(options.type as FieldType);
    ...
    const fieldControls = [
        { id: 'entryTitle', widget: 'singleLine', helpText: 'The title of the IG Grid D Section entry' },
        { id: 'title', widget: 'singleLine', helpText: 'Localized title for the IG Grid D Section' },
        ...
        { id: 'platformVisibility', widget: 'checkbox', helpText: 'Platforms where this IG Grid D Section is visible' },
        { id: 'sessionVisibility', widget: 'checkbox', helpText: 'Sessions where this IG Grid D Section is applicable' },
        ...
        { id: 'layoutType', widget: 'dropdown', helpText: 'Layout type (fixed to grid-d)' },
    ...
    fieldControls.forEach(({ id, widget, helpText }) => {
        igGridDSection.changeFieldControl(id, 'builtin', widget as BuiltinEditor, { helpText });
    });
