...
export = ((migration) => {
    const igCollabBasedPersonalisedSection = migration
        .createContentType('igCollabBasedPersonalisedSection')
        .name('IG Collab Based Personalised Section')
        .description('For Collab Based Suggestions coming from ML')
        .displayField('entryTitle');

    const igCollabBasedPersonalisedSectionFields: { id: string, options: IFieldOptions }[] = [
        ...
        { id: 'type', options: { name: 'Type', type: 'Symbol', required: true, validations: [{ in: ['personal-suggested-games', 'on-exit-recommendations'] }], defaultValue: { [locale]: 'personal-suggested-games' } } },
    ...
    igCollabBasedPersonalisedSectionFields.forEach(field => {
        const { id, options } = field;
        const fieldInstance = igCollabBasedPersonalisedSection.createField(id).name(options.name as string).type(options.type as FieldType);
    ...
    const fieldControls = [
        ...
        { id: 'type', widget: 'dropdown', helpText: 'Type of Collab-based recommendations' },
        { id: 'games', widget: 'entryLinksEditor', helpText: 'Fallback games' },
    ...
    fieldControls.forEach(({ id, widget, helpText }) => {
        igCollabBasedPersonalisedSection.changeFieldControl(
            id,
            'builtin',
