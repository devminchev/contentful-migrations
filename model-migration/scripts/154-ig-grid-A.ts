...
export = ((migration) => {
    const igGridASection = migration
        .createContentType('igGridASection')
        .name('IG Grid A Section')
        .description('')
        .displayField('entryTitle');

    const igGridAFields: { id: string, options: IFieldOptions }[] = [
        ...
        { id: 'layoutType', options: { name: 'Layout Type', type: 'Symbol', required: false, defaultValue: { [locale]: 'grid-a' }, validations: [{ in: ['grid-a'] }], disabled: true } },
    ...
    igGridAFields.forEach(field => {
        const { id, options } = field;
        const fieldInstance = igGridASection.createField(id).name(options.name as string).type(options.type as FieldType);
        ...
        if (options.omitted     !== undefined) fieldInstance.omitted(options.omitted);
    });

    const fieldControls = [
        { id: 'entryTitle', widget: 'singleLine', helpText: 'The title of the IG Grid A Section entry' },
        { id: 'title', widget: 'singleLine', helpText: 'Localized title for the IG Grid A Section' },
        ...
        { id: 'platformVisibility', widget: 'checkbox', helpText: 'Platforms where this IG Grid A Section is visible' },
        { id: 'sessionVisibility', widget: 'checkbox', helpText: 'Sessions where this IG Grid A Section is applicable' },
        ...
        { id: 'layoutType', widget: 'singleLine', helpText: 'Layout type (fixed to grid-a)' },
    ...
    fieldControls.forEach(({ id, widget, helpText }) => {
        igGridASection.changeFieldControl(id, 'builtin', widget as BuiltinEditor, { helpText });
    });
