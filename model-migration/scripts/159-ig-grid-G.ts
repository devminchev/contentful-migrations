...
export = ((migration) => {
    const igGridGSection = migration
        .createContentType('igGridGSection')
        .name('IG Grid G Section')
        .description('')
        .displayField('entryTitle');

    const igGridGFields: { id: string, options: IFieldOptions }[] = [
        ...
        { id: 'layoutType', options: { name: 'Layout Type', type: 'Symbol', required: false, defaultValue: { [locale]: 'grid-g' }, validations: [{ in: ['grid-g'] }], disabled: true } },
    ...
    igGridGFields.forEach(field => {
        const { id, options } = field;
        const fieldInstance = igGridGSection.createField(id).name(options.name as string).type(options.type as FieldType);
    ...
    const fieldControls = [
        { id: 'entryTitle', widget: 'singleLine', helpText: 'The title of the IG Grid G Section entry' },
        { id: 'title', widget: 'singleLine', helpText: 'Localized title for the IG Grid G Section' },
        ...
        { id: 'platformVisibility', widget: 'checkbox', helpText: 'Platforms where this IG Grid G Section is visible' },
        { id: 'sessionVisibility', widget: 'checkbox', helpText: 'Sessions where this IG Grid G Section is applicable' },
        ...
        { id: 'layoutType', widget: 'dropdown', helpText: 'Layout type (fixed to grid-g)' },
    ...
    fieldControls.forEach(({ id, widget, helpText }) => {
        igGridGSection.changeFieldControl(id, 'builtin', widget as BuiltinEditor, { helpText });
    });
